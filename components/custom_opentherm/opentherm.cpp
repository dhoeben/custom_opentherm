...
static float ch_heat_flow_for_preset(
    ChPreset preset,
    float equitherm_target,
    float max_ch_temp
) {
    switch (preset) {
        case ChPreset::ECO: {
            float eco_max = std::clamp(
                max_ch_temp - 15.0f,
                30.0f,
                max_ch_temp
            );
...
void OpenThermComponent::schedule_control_requests_() {
    ...
    float equitherm_target = emergency_.is_active()
        ? emergency_.target_c()
    ...
    if (!emergency_.is_active() && equitherm_target < IDLE_THRESHOLD_C) {
        target_c = ch_idle_flow_for_preset(ch_preset_mapped_);
    } else if (!emergency_.is_active()) {
        target_c = ch_heat_flow_for_preset(
            ch_preset_mapped_,
            equitherm_target,
    ...
    if (dhw_preheat_enabled_ != last_sent_dhw_preheat_enabled_) {
        enqueue_write_(OT_MSG_DHW_COMFORT, dhw_preheat_enabled_ ? 1u : 0u);
...
void OpenThermComponent::enqueue_read_(uint8_t did) {
    ...
    low_prio_queue_.push({RequestKind::Read, RequestPriority::Low, did, 0});
}

void OpenThermComponent::enqueue_write_(uint8_t did, uint16_t data) {
    high_prio_queue_.push({RequestKind::Write, RequestPriority::High, did, data});
}
...
void OpenThermComponent::process_queue_() {
    ...
    Request req = !high_prio_queue_.empty()
        ? high_prio_queue_.front()
        : low_prio_queue_.front();
...
void OpenThermComponent::update_comms_ok_(uint32_t now_ms) {
    comms_ok_ = (last_valid_rx_ms_ != 0) &&
                (now_ms - last_valid_rx_ms_ <= COMMS_TIMEOUT_MS);
}

void OpenThermComponent::update_mode_text_() {
    if (mode_text_ != nullptr)
        mode_text_->publish_state(
            emergency_.is_active() ? "emergency" : "equitherm"
        );
}
...
void OpenThermComponent::publish_() {
    if (queue_depth_sensor_ != nullptr)
        queue_depth_sensor_->publish_state(
            static_cast<float>(
                high_prio_queue_.size() + low_prio_queue_.size()
            )
        );
    ...
    if (outside_temp_sensor_ != nullptr && outside_valid_)
        outside_temp_sensor_->publish_state(last_outside_c_);
    if (setpoint_sensor_ != nullptr)
        setpoint_sensor_->publish_state(last_target_setpoint_c_);
    ...
    if (dhw_preheat_binary_ != nullptr)
        dhw_preheat_binary_->publish_state(dhw_.preheat_active());
